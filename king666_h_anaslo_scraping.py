"""
キング666  半田店 アナスロスクレイピングスクリプト
- スクレイピングロジック: アナスロスクレイピング_対策版_差枚あり_ARTなし.py を参考
- Docker/Proxy/DB/エクスポート部分: ogiya_pscube_slot_scraping.py と同じ構造
- データ保存先: slot_data_king666_h.db
- エクスポート先: docs/king666_h/data.json, docs/king666_h/layout.json
"""
...
# ==========================================
# アナスロ キング666半田店URL
ANASLO_BASE_URL = 'https://ana-slo.com/%E3%83%9B%E3%83%BC%E3%83%AB%E3%83%87%E3%83%BC%E3%82%BF/%E6%84%9B%E7%9F%A5%E7%9C%8C/%E3%82%AD%E3%83%B3%E3%82%B0666%E5%8D%8A%E7%94%B0%E5%BA%97-%E3%83%87%E3%83%BC%E3%82%BF%E4%B8%80%E8%A6%A7/'
...
DB_FILE = os.path.join(SCRIPT_DIR, "slot_data_king666_h.db")
DOCS_DIR = os.path.join(SCRIPT_DIR, "docs", "king666_h")
...
def is_date_complete(date_str: str, expected_count: int = 400) -> bool:
...
def parse_scraped_data(scraped_list: list, date_str: str) -> list:
    """
    アナスロのスクレイピングデータ(11要素/行: 機種名, 台番号, G数, 差枚, BIG, REG, ...)
    をDBレコード用dictに変換する。
    """
    records = []
    chunk_size = 11
...
# スクレイピング本体 (アナスロ方式)
# ==========================================
def scrape_king666_h_anaslo(
    start_date: datetime.datetime,
    end_date: datetime.datetime,
    stop_event: threading.Event = None,
) -> list:
    """
    アナスロからキング666半田のデータをスクレイピングして
    DBレコードのリストを返す。
    """
    co = ChromiumOptions()
    co.headless(False)
...
def export_and_upload_to_github():
    ...
    try:
        export_script = "export_slot_data_king666_h.py"
        if os.path.exists(os.path.join(SCRIPT_DIR, export_script)):
            subprocess.run([sys.executable, export_script], check=True)
            logger.info("JSONデータのエクスポートが完了しました。")
        else:
            logger.warning(f"エクスポートスクリプト {export_script} が見つかりません。")
        ...
        commit_res = subprocess.run(
            ["git", "commit", "-m",
             f"Auto update king666_h data {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"],
            capture_output=True, text=True
        )
...
def main(skip_scraping: bool = False):
    logger.info("=== キング666東海店 スクレイピング開始 ===")
    init_db()
    ...
    if not skip_scraping:
        ...
        for attempt in range(max_restarts):
            try:
                ...
                scrape_king666_h_anaslo(start_date, end_date)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="King 666 Handa Anaslot Scraper")

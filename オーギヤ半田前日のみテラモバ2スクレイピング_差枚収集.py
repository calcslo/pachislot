...
def main():
    ...
    def check_and_write_dates(last_date_text, first_date_text, flag):
        last_date_file = r"C:\Users\mikih\PycharmProjects\pythonProject\S_ogiyahanda_last_date.txt"
        if not flag:
            """
    ...
    try:
        ...
        for i in range(674, 1386):
            ...
            try:
                ...
                pass
            first_date = page.ele(f'css:#sel_target_date > option:nth-child(2)')
            try:
                if first_date:
            ...
            for d in range(kaisu):
                ...
                try:
                    ...
                    dedama_sum = [int(item) if item != "-" else 0 for item in dedama]
                    dedama_sum = sum(dedama_sum)
                    ...
                    final_start = int(page.ele('css:#contents-container > article > section.box-base.machine-info > div.box-article01 > div.banBox.digitPanel > div.tables.doubleBox.topBorder.dailist_boxheight_slot > div.left.start > div > div.daidigit.num.s.green').text)
                    samai = cal_samai(path, max_mochidama)[0]
                    samai2 = cal_samai_from_bonus_history(start, dedama, kishumei, final_start)
                    win_flag = cal_samai(path, max_mochidama)[1]
                    print(f"最終差枚：{samai}(検定用予測差枚：{samai2})")
                    threshold = abs(samai2) * 0.6  # samai2の絶対値の20%
                    lower_bound = samai2 - threshold  # 下限
                    upper_bound = samai2 + threshold  # 上限
                    if max_mochidama >= 1000:
                        samai2 = samai
                    if win_flag and samai <= 0:
                        samai2 = samai
                    if not win_flag and samai >= 0:
                        samai2 = samai
                    if samai2 > dedama_sum:
                        samai2 = samai
                    if samai < lower_bound or samai > upper_bound:
                        samai = samai2
                    if samai > dedama_sum:
                        samai = None
                    print(f"最終差枚：{samai}(検定用予測差枚：{samai2})")
                    start_sum = int(page.ele("css:#contents-container > article > section.box-base.machine-info > div.box-article01 > div.banBox.digitPanel > div.tables.doubleBox.topBorder.dailist_boxheight_slot > div.right.soukaiten > div > div.daidigit.num.s.green.data-soukaiten").text)
                except Exception as e:
        ...
        if not break_flag:
            ...
            store_name = "オーギヤ半田"
            # CSVファイルに書き出し
            filename = r"C:\Users\mikih\PycharmProjects\pythonProject\S_sadamadata" + store_name + ".csv"
        ...
        filename = os.path.join(log_dir, today_str + "error1.txt")

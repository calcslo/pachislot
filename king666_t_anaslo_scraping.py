"""
キング666東海店 アナスロスクレイピングスクリプト
- スクレイピングロジック: アナスロスクレイピング_対策版_差枚あり_ARTなし.py を参考
- Docker/Proxy/DB/エクスポート部分: ogiya_pscube_slot_scraping.py と同じ構造
- データ保存先: slot_data_king666_t.db
- エクスポート先: docs/king666_t/data.json, docs/king666_t/layout.json
"""
...
# ==========================================
# アナスロ キング666東海店URL
ANASLO_BASE_URL = 'https://ana-slo.com/%E3%83%9B%E3%83%BC%E3%83%AB%E3%83%87%E3%83%BC%E3%82%BF/%E6%84%9B%E7%9F%A5%E7%9C%8C/%E3%82%AD%E3%83%B3%E3%82%B0666%E6%9D%B1%E6%B5%B7%E5%BA%97-%E3%83%87%E3%83%BC%E3%82%BF%E4%B8%80%E8%A6%A7/'
...
DOCS_DIR = os.path.join(SCRIPT_DIR, "docs", "king666_t")
...
# スクレイピング本体 (アナスロ方式)
# ==========================================
def scrape_king666_t_anaslo(
    start_date: datetime.datetime,
    end_date: datetime.datetime,
    stop_event: threading.Event = None,
) -> list:
    """
    アナスロからキング666東海店のデータをスクレイピングして
    DBレコードのリストを返す。
    """
    co = ChromiumOptions()
    co.headless(False)
...
def export_and_upload_to_github():
    ...
    try:
        export_script = "export_slot_data_king666_t.py"
        ...
        commit_res = subprocess.run(
            ["git", "commit", "-m",
             f"Auto update king666_t data {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"],
            capture_output=True, text=True
        )
    ...
    if not skip_scraping:
        ...
        for attempt in range(max_restarts):
            try:
                ...
                scrape_king666_t_anaslo(start_date, end_date)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="King 666 Tokai Anaslot Scraper")

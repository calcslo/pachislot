...
def main():
    ...
    def check_and_write_dates(last_date_text, first_date_text, flag):
        last_date_file = r"C:\Users\mikih\PycharmProjects\pythonProject\P_ogiyahanda_last_date.txt"
        if not flag:
            """
    ...
    try:
        ...
        for i in chain(*ranges):
            ...
            for d in range(kaisu):
                try:
                    select_date = page.ele(f'css:#sel_target_date > option:nth-child({d + 1})', timeout=3)
                ...
                try:
                    ...
                    BB_kaisu = int(page.ele("css:#contents-container > article > section.box-base.machine-info > div.box-article01 > div.banBox.digitPanel > div:nth-child(2) > div.left.atari > div > div.daidigit.num.l.red.data-atari").text)

                    if start_sum == 0 or BB_kaisu == 0:
                        m_kaitensuu = 0
                        M_kaitensuu = 0
                        kaitensuu = 0
                        sadama = 0
                        ...
                        all_scraped_data.append(
                            [date, kishumei, dai_num, m_kaitensuu, kaitensuu, M_kaitensuu, sadama, m_kitaichi, kitaichi,
                             M_kitaichi, start_sum])
                        continue
                except:
                    pass
                try:
        ...
        if not break_flag:
            ...
            store_name = "オーギヤ半田"
            # CSVファイルに書き出し
            filename = r"C:\Users\mikih\PycharmProjects\pythonProject\P_sadamadata" + store_name + ".csv"

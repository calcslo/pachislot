...
from オーギヤ半田パチンコ出玉履歴から差枚計算 import cal_samai_from_bonus_history
from オーギヤ半田パチンコ出玉履歴から差枚計算 import m_cal_samai_from_bonus_history
from オーギヤ半田パチンコ出玉履歴から差枚計算 import M_cal_samai_from_bonus_history
...
def main():
    ...
    def check_and_write_dates(last_date_text, first_date_text):
        last_date_file = "ogiyahanda_last_date.txt"
        first_date_file = "ogiyahanda_first_date.txt"
        flag = False
        """
        ...
        if os.path.exists(last_date_file) and os.path.exists(first_date_file):
    ...
    try:
        ...
        for i in chain(*ranges):
            ...
            try:
                ...
                pass
            first_date = page.ele(f'css:#sel_target_date > option:nth-child(7)')
            try:
                if first_date:
                ...
                pass
            try:
                if (last_date_text and first_date_text) and not check_and_write_dates(last_date_text, first_date_text):
                    break_flag = True
            ...
            for d in range(kaisu):
                try:
                    ...
                    if select_date and select_date.states.is_displayed:
                        ...
                        print(f"Select date option {d + 2} not displayed.")
                ...
                try:
                    ...
                    csfb_list = cal_samai_from_bonus_history(start, dedama, kishumei, final_start, path, shubetu, max_mochidama)
                    m_csfb_list = m_cal_samai_from_bonus_history(start, dedama, kishumei, final_start, path, shubetu, max_mochidama)
                    M_csfb_list = M_cal_samai_from_bonus_history(start, dedama, kishumei, final_start, path, shubetu, max_mochidama)

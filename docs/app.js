...
const MACHINE_PROBS = {
    'LB・・ｽｭ・ｰ・ｷ・晢ｽｸ・橸ｾ奇ｾ・ｾ奇ｾ・': { 1: { big: 299, reg: 496 }, 2: { big: 291, reg: 471 }, 3: { big: 281, reg: 442 }, 4: { big: 268, reg: 409 }, 5: { big: 253, reg: 372 } },
    '・ｺ・橸ｽｰ・ｺ・橸ｽｰ・ｼ・橸ｽｬ・ｸ・橸ｾ暦ｽｰ3': { 1: { big: 259, reg: 354.2 }, 2: { big: 258, reg: 332.7 }, 3: { big: 257, reg: 306.2 }, 4: { big: 254, reg: 268.6 }, 5: { big: 247.3, reg: 247.3 }, 6: { big: 234.9, reg: 234.9 } },
    '・ｳ・呻ｾ・ｾ暦ｾ撰ｾ暦ｽｸ・呻ｽｼ・橸ｽｬ・ｸ・橸ｾ暦ｽｰ': { 1: { big: 267.5, reg: 425.6 }, 2: { big: 261.1, reg: 402.1 }, 3: { big: 256, reg: 350.5 }, 4: { big: 242.7, reg: 322.8 }, 5: { big: 233.2, reg: 297.9 }, 6: { big: 216.3, reg: 277.7 } },
    '・ｼ・橸ｽｬ・ｸ・橸ｾ暦ｽｰ・ｶ・橸ｽｰ・呻ｽｽ・朶S': { 1: { big: 273.1, reg: 381 }, 2: { big: 270.8, reg: 350.5 }, 3: { big: 260.1, reg: 316.6 }, 4: { big: 250.1, reg: 281.3 }, 5: { big: 243.6, reg: 270.8 }, 6: { big: 226, reg: 252.1 } },
    '・撰ｽｽ・・ｰ・ｼ・橸ｽｬ・ｸ・橸ｾ暦ｽｰ': { 1: { big: 268.6, reg: 374.5 }, 2: { big: 267.5, reg: 354.2 }, 3: { big: 260.1, reg: 331 }, 4: { big: 249.2, reg: 291.3 }, 5: { big: 240.9, reg: 257 }, 6: { big: 237.4, reg: 237.4 } }
};
...
let includeOldData = false; // 譌ｧ繝・・繧ｿ(2026/4/21莉･蜑・繧貞性繧√ｋ縺九←縺・°
const OLD_DATA_CUTOFF = '2026-04-21'; // 縺薙・譌･莉倅ｻ･蜑阪・繝・・繧ｿ繧呈立繝・・繧ｿ縺ｨ縺励※謇ｱ縺・
...
function getPosLabel(num) { const l = layoutLookup[num]; if (!l) return '荳肴・'; if ((parseInt(num) >= 987 && parseInt(num) <= 998) || (parseInt(num) >= 1370 && parseInt(num) <= 1385)) return '蜀・ｽ｢蟲ｶ'; return l.pos === 0 ? '隗・ : l.pos === 1 ? '隗・' : l.pos === 2 ? '隗・' : l.pos === null ? '蜀・ｽ｢蟲ｶ' : '縺昴・莉・; }
...
function buildLayoutLookup() {
    ...
    for (const cell of Object.keys(layoutLookup)) {
        ...
        if (numVal >= 987 && numVal <= 998) {
            layoutLookup[cell].islandId = '蜀・ｽ｢蟲ｶ(987-998)'; layoutLookup[cell].islandMin = 987;
        } else if (numVal >= 1370 && numVal <= 1385) {
            layoutLookup[cell].islandId = '蜀・ｽ｢蟲ｶ(1370-1385)'; layoutLookup[cell].islandMin = 1370;
...
function setupEventListeners() {
    ...
    document.getElementById('old-data-toggle').addEventListener('click', () => {
        ...
        if (includeOldData) {
            ...
            btn.textContent = '譌ｧ繝・・繧ｿ ON';
        } else {
            btn.classList.remove('active');
            btn.textContent = '譌ｧ繝・・繧ｿ OFF';
        }
        activeDate = null;
        updateDashboard();
    });

"""
kyoraku東海店 アナスロスクレイピングスクリプト
- スクレイピングロジック: アナスロスクレイピング_対策版_差枚あり_ARTなし.py を参考
- Docker/Proxy/DB/エクスポート部分: ogiya_pscube_slot_scraping.py と同じ構造
- データ保存先: slot_data_kyoraku_t.db
- エクスポート先: docs/kyoraku_t/data.json, docs/kyoraku_t/layout.json
"""
...
# ==========================================
# アナスロ kyoraku東海店URL
ANASLO_BASE_URL = 'https://ana-slo.com/%E3%83%9B%E3%83%BC%E3%83%AB%E3%83%87%E3%83%BC%E3%82%BF/%E6%84%9B%E7%9F%A5%E7%9C%8C/kyoraku%E6%9D%B1%E6%B5%B7%E5%BA%97-%E3%83%87%E3%83%BC%E3%82%BF%E4%B8%80%E8%A6%A7/'
...
DB_FILE = os.path.join(SCRIPT_DIR, "slot_data_kyoraku_t.db")
DOCS_DIR = os.path.join(SCRIPT_DIR, "docs", "kyoraku_t")
...
def is_date_complete(date_str: str, expected_count: int = 200) -> bool:
...
def parse_scraped_data(scraped_list: list, date_str: str) -> list:
    """
    アナスロのスクレイピングデータ(9要素/行: 機種名, 台番号, G数, 差枚, BIG, REG, ...)
    をDBレコード用dictに変換する。
    """
    records = []
    chunk_size = 11
...
# スクレイピング本体 (アナスロ方式)
# ==========================================
def scrape_kyoraku_t_anaslo(
    start_date: datetime.datetime,
    end_date: datetime.datetime,
    stop_event: threading.Event = None,
) -> list:
    """
    アナスロからkyoraku東海店のデータをスクレイピングして
    DBレコードのリストを返す。
    """
    co = ChromiumOptions()
    co.headless(False)
...
def export_and_upload_to_github():
    ...
    try:
        # エクスポートスクリプトを実行
        export_script = "export_slot_data_kyoraku_t.py"
        ...
        commit_res = subprocess.run(
            ["git", "commit", "-m",
             f"Auto update kyoraku_t data {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"],
            capture_output=True, text=True
        )
...
def main(skip_scraping: bool = False):
    logger.info("=== kyoraku東海店 スクレイピング開始 ===")
    init_db()
    ...
    if not skip_scraping:
        ...
        for attempt in range(max_restarts):
            try:
                ...
                scrape_kyoraku_t_anaslo(start_date, end_date)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kyoraku Tokai Anaslot Scraper")

...
def build_features(df, layout_lookup):
    ...
    for col in ['cons_neg', 'cons_pos', 'prev_setting_1', 'prev_high_setting_1', 'same_wd_avg_diff', 'same_wd_win_rate', 'store_high_ratio'] + spatial_cols:
...
def analyze_feature_importance(X, y, task='regression'):
    ...
    if task == 'regression':
        ...
        # 実験で最適化されたパラメータ（F1最大化）
        model = XGBClassifier(
            n_estimators=454, max_depth=4, learning_rate=0.005561,
            reg_lambda=29.565, reg_alpha=5.111, subsample=0.5676, colsample_bytree=0.5000,
            gamma=1.220, min_child_weight=9, tree_method='hist', device='cuda',
    ...
    if task == 'regression':
        ...
        # 評価用閾値: 全体のトップ10%（約30台/日）を予想ターゲットとした場合の実力
        best_th = np.percentile(oof_prob, 90)
...
def predict_next_day(df_feat):
    ...
    # Feature Selection (不要な特徴量の除外)
    # 事前にXGBoostで重要度を計算し、下位20%の特徴量を除外する
    from sklearn.ensemble import RandomForestClassifier
    fs_model = RandomForestClassifier(n_estimators=100, max_depth=5, random_state=42, n_jobs=-1)
    fs_model.fit(X_train_cls, y_train_cls)
    importances = fs_model.feature_importances_
    
    # カテゴリ変数は除外しないように保護
    protected_cols = set(cat_cols_present)
    
    feature_imp = [(feat_cols[i], imp) for i, imp in enumerate(importances)]
    feature_imp.sort(key=lambda x: x[1], reverse=True)
    
    # 下位20%の数を計算
    drop_count = int(len(feature_imp) * 0.2)
    features_to_keep = []
    
    for feat, imp in feature_imp[:-drop_count]:
    ...
    # 特徴量リストを更新
    X_train_cls = X_train_cls[features_to_keep]
    df_next_selected = df_next[features_to_keep]
    
    xgb_oof = np.zeros(len(X_train_cls))
    lgb_oof = np.zeros(len(X_train_cls))
    cat_oof = np.zeros(len(X_train_cls))
    
    xgb_preds_folds = np.zeros(len(df_next))
    lgb_preds_folds = np.zeros(len(df_next))
    cat_preds_folds = np.zeros(len(df_next))
    
    # K-Fold Stacking / Averaging
    for train_idx, val_idx in skf.split(X_train_cls, y_train_cls):
        X_tr, y_tr = X_train_cls.iloc[train_idx], y_train_cls.iloc[train_idx]
        X_va, y_va = X_train_cls.iloc[val_idx], y_train_cls.iloc[val_idx]
        
        # 1. XGBoost
        model_xgb = XGBClassifier(
            n_estimators=454, max_depth=4, learning_rate=0.005561,
            reg_lambda=29.565, reg_alpha=5.111, subsample=0.5676, colsample_bytree=0.5000,
            gamma=1.220, min_child_weight=9, tree_method='hist', device='cuda', random_state=42,
            scale_pos_weight=spw
        )
        model_xgb.fit(X_tr, y_tr)
        xgb_oof[val_idx] = model_xgb.predict_proba(X_va)[:, 1]
        xgb_preds_folds += model_xgb.predict_proba(df_next_selected)[:, 1] / 5.0
        
        # 2. LightGBM
        model_lgb = lgb.LGBMClassifier(
            n_estimators=358, max_depth=2, learning_rate=0.019269,
            reg_lambda=29.828, reg_alpha=1.500, subsample=0.9057, colsample_bytree=0.9658,
            min_child_samples=14, num_leaves=30, random_state=42, verbose=-1,
            scale_pos_weight=spw
        )
        model_lgb.fit(X_tr, y_tr)
        lgb_oof[val_idx] = model_lgb.predict_proba(X_va)[:, 1]
        lgb_preds_folds += model_lgb.predict_proba(df_next_selected)[:, 1] / 5.0
        
        # 3. CatBoost
        model_cat = CatBoostClassifier(
            iterations=300, depth=6, learning_rate=0.03,
            cat_features=cat_cols_present, verbose=0, random_state=42,
            auto_class_weights='Balanced'
        )
        model_cat.fit(X_tr, y_tr)
        cat_oof[val_idx] = model_cat.predict_proba(X_va)[:, 1]
        cat_preds_folds += model_cat.predict_proba(df_next_selected)[:, 1] / 5.0
        
    # アンサンブル (単純平均)
    oof_probs = (xgb_oof + lgb_oof + cat_oof) / 3.0
    pred_probs = (xgb_preds_folds + lgb_preds_folds + cat_preds_folds) / 3.0

    # 評価用閾値: 全体のトップ10%（約30台/日に相当）を抽出する閾値
    # ユーザーにはトップ30台を提示するため、この「トップ10%ライン」の精度が、提示する台の期待値と正確に一致する
    best_threshold = np.percentile(oof_probs, 90)
    pred_high = (oof_probs >= best_threshold).astype(int)
    
    best_hit_rate = precision_score(y_train_cls, pred_high, zero_division=0)
    selected_diffs = df_cls['最終差枚'][pred_high == 1]

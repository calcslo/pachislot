...
def load_sheet_data(sheet_name):
    df = pd.DataFrame(wb[sheet_name].values, columns=["台番号", "予測差枚合計"])
    df["予測差枚合計"] = pd.to_numeric(df["予測差枚合計"], errors="coerce")
    return df
...
def calculate_ranges(df):
    # プラスとマイナスの最小値・最大値を計算
    min_value = df["予測差枚合計"].min()
    max_value = df["予測差枚合計"].max()
    ...
    for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row, min_col=1, max_col=sheet.max_column):
        for cell in row:
            if cell.value in df_data["台番号"].values:
                # 差枚の値を取得
                diff = df_data.loc[df_data["台番号"] == cell.value, "予測差枚合計"].values[0]
